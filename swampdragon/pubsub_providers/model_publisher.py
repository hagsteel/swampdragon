...
def publish_model(model_instance, serializer, action, changed_fields=None):
    ...
    previous_channels = filter_channels_by_dict(all_model_channels, model_instance._pre_save_state)
    remove_from_channels = set(previous_channels) - set(channels)
    ...
    if changed_fields and remove_from_channels:
